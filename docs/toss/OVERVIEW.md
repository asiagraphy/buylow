> 원본 URL: https://openapi.tossinvest.com/openapi-docs/overview.md
...
토스증권 Open API 는 **REST API** 와 **웹소켓**(실시간 시세·주문 이벤트 구독)을 제공합니다. 웹소켓 연동 방법은 아래 [웹소켓 연동](OVERVIEW.md#%EC%9B%B9%EC%86%8C%EC%BC%93-%EC%97%B0%EB%8F%99) 섹션을 참고하세요.
...
| 엔드포인트 | 설명 |
|------|------|
| `GET /ws/v1` (웹소켓) | 실시간 체결·호가·본인 주문 이벤트 구독 — [웹소켓 연동](OVERVIEW.md#%EC%9B%B9%EC%86%8C%EC%BC%93-%EC%97%B0%EB%8F%99) 참고 |
...
| 401 UNAUTHORIZED          | `token-revoked`                      | 새로 발급된 토큰으로 대체되어 이전 토큰이 무효화되었습니다. 토큰은 클라이언트당 1개만 유효하니 최신 토큰으로 다시 시도하세요. |
...
| 409 CONFLICT              | `opposite-pending-order-exists`      | 동일 종목에 반대 방향의 체결 대기 주문이 존재합니다. |
