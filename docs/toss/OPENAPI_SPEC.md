> 원본 URL: https://openapi.tossinvest.com/openapi-docs/latest/openapi.json
> 문서 버전: 1.2.17
...
````json
{
  ...
  "info": {
    "title": "토스증권 Open API",
    "version": "1.2.17",
    "description": "토스증권 트레이딩 Open API 의 REST 스펙입니다. 실시간 체결·호가·본인 주문 이벤트 스트리밍은 REST 가 아닌 웹소켓 API(`wss://openapi-ws.tossinvest.com/ws/v1`)로 제공되며, 별도의 AsyncAPI 3.0 문서 (https://openapi.tossinvest.com/openapi-docs/latest/asyncapi.json)가 실시간 스펙의 source of truth 입니다."
  },
  "servers": [
    {
  ...
  "tags": [
    ...
    {
      "name": "Market Data",
      "description": "종목의 실시간성 시세 정보를 조회하는 그룹입니다. 매수/매도 호가, 현재가, 최근 체결 내역, 상·하한가, 캔들 차트(1분봉·일봉)를 제공합니다. 계좌 정보 없이 액세스 토큰만으로 호출할 수 있습니다. 호가·현재가·체결·상하한가는 `MARKET_DATA` Rate Limits Group 에 속하고, 캔들 차트는 호출 부하 특성이 달라 별도의 `MARKET_DATA_CHART` Rate Limits Group 으로 관리되므로, 두 그룹의 한도 응답 헤더를 각각 확인하는 것을 권장합니다.\n\n**이용 안내**\n- 실시간 체결·호가는 폴링 대신 웹소켓 API(`wss://openapi-ws.tossinvest.com/ws/v1`)로 구독할 수 있습니다 — AsyncAPI 스펙(`/openapi-docs/latest/asyncapi.json`)과 연동 가이드의 **웹소켓 연동** 섹션 참고"
    },
    {
      "name": "Stock Info",
      "description": "종목명·시장·통화·상장 상태·발행주식수 등 종목의 기본 참조 정보와 매수 유의사항, 그리고 국내(KR) 종목의 수급 동향(투자자별 매매동향·프로그램매매·공매도·신용거래·대차거래)을 제공하는 그룹입니다. 기본 참조 정보와 매수 유의사항은 영업일 단위(또는 그 이상)로 갱신되므로 짧은 주기로 폴링하지 말고 화면·세션 진입 시점에 한 번 캐시해 사용하기를 권장합니다. 수급 동향 시계열은 데이터별로 반영 시점이 다르므로(장중 잠정치 ~ T+1) 각 엔드포인트 설명의 데이터 적시성 안내를 참조하세요."
    },
    {
      "name": "Market Info",
  ...
  "externalDocs": {
    "description": "실시간 웹소켓 API 스펙 (AsyncAPI 3.0) — 구독 선언·메시지 스키마·한도·keepalive·재연결 규칙",
    "url": "https://openapi.tossinvest.com/openapi-docs/latest/asyncapi.json"
  },
  "paths": {
    "/oauth2/token": {
      "post": {
